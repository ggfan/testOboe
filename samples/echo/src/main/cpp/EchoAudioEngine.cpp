...
void EchoAudioEngine::openAllStreams() {
  ...
  if (recordingStream_ && playStream_) {
    mixAudio_ = mixerEffect_->AudioFormatSupported(
        playStream_->getSampleRate(), playStream_->getChannelCount(),
        playStream_->getFormat());
...
void EchoAudioEngine::openPlaybackStream() {
  ...
  if (result == oboe::Result::OK && playStream_) {
    ...
    systemWarmupFrames_ =
        static_cast<uint64_t>(sampleRate_ * kSystemWarmupTime);
...
oboe::DataCallbackResult EchoAudioEngine::onAudioReady(
  ...
  oboe::Result errorCode = oboe::Result::OK;
  int32_t framesRead = 0;
  if (processedFrameCount_ < systemWarmupFrames_) {
    int32_t prevFrameRead = 0;
    do {
  ...
  if (errorCode != oboe::Result::OK) {
    LOGE("input stream read error:  %s", oboe::convertToText(errorCode));
    LOGE("echo is stopped");
  ...
  delayEffect_->process(static_cast<int16_t *>(audioData), outputChannelCount_,
                        numFrames);
  if (mixAudio_) {
...
void EchoAudioEngine::setBackgroundStream(std::unique_ptr<int16_t[]> samples,
                                          size_t sampleCount,
                                          int32_t sampleRate,
