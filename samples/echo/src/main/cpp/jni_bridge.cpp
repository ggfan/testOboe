...
JNIEXPORT void JNICALL
Java_com_google_sample_oboe_echo_EchoEngine_setPlaybackDeviceId(JNIEnv *env,
                                                                jclass,
                                                                jint deviceId) {
  if (engine == nullptr) {
    LOGE(
...
Java_com_google_sample_oboe_echo_EchoEngine_setStreamFile(
  ...
  AAsset *sampleAsset =
      AAssetManager_open(jniAssetManager, jniFileName, AASSET_MODE_UNKNOWN);
  size_t sampleCount = static_cast<size_t>(AAsset_getLength(sampleAsset) / 2);
  ...
  engine->setBackgroundStream(std::move(samples), sampleCount, sampleRate,
                              channelCount);
...
JNIEXPORT void JNICALL Java_com_google_sample_oboe_echo_EchoEngine_setMixer(
...
JNIEXPORT void JNICALL
Java_com_google_sample_oboe_echo_EchoEngine_setEchoControls(JNIEnv *env,
                                                            jclass type,
                                                            jfloat delay,
                                                            jfloat decay) {
  if (engine == nullptr) {
...
JNIEXPORT jboolean JNICALL Java_com_google_sample_oboe_echo_EchoEngine_setAPI(
...
Java_com_google_sample_oboe_echo_EchoEngine_isAAudioSupported(JNIEnv *env,
                                                              jclass type) {
  if (engine == nullptr) {
    LOGE(
        "Engine is null, you must call createEngine "
