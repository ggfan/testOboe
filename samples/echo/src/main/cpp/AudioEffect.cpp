...
#define CLAMP_FOR_I16(x) \
  ((x) > SHRT_MAX ? SHRT_MAX : ((x) < SHRT_MIN ? SHRT_MIN : (x)))
...
AudioMixer::AudioMixer() : AudioFormat(48000, 2, oboe::AudioFormat::I16) {
  bgMixFactorInt_ = (int32_t)(bgMixFactor_ * kFloatToIntMapFactor + 0.5f);
 ...
 */
bool AudioMixer::addStream(std::unique_ptr<int16_t[]> samples,
                           size_t sampleCount, int32_t sampleRate,
                           int32_t channelCount, oboe::AudioFormat format) {
...
void AudioMixer::process(int16_t *liveAudio, int32_t channelCount,
  ...
  if ((numFrames * channelCount) > bgAudioSampleCount_ ||
      channelCount != channelCount_ || bgMixFactorInt_ == 0) {
    return;
  }
  ...
  for (int i = 0; i < sampleCount; i++) {
    ...
    curSample =
        curSample * fgMixFactorInt_ + bgAudio_[curPosition_] * bgMixFactorInt_;
 ...
 */
bool AudioMixer::AudioFormatSupported(int32_t sampleRate, int32_t channels,
...
AudioDelay::AudioDelay(int32_t sampleRate, int32_t channelCount,
                       oboe::AudioFormat format, float delay, float decay)
 ...
 * Destructor
 */
AudioDelay::~AudioDelay() { delete buffer_; }
...
float AudioDelay::getDelay(void) const { return delay_; }
...
float AudioDelay::getDecay(float) const { return decay_; }
 ...
 */
void AudioDelay::process(int16_t *liveAudio, int32_t channelCount,
                         int32_t numFrames) {
  if (!buffer_ || !liveAudio || feedbackFactor_ == 0 ||
      channelCount != channelCount_ || bufSize_ < numFrames) {
    return;
  }
  ...
  int16_t *samples =
      &reinterpret_cast<int16_t *>(buffer_)[curPos_ * channelCount_];
